...
| `tnc2` | Lossless TNC2-compatible textual envelope and opaque address model. It deliberately has no AX.25 dependency or bounded numeric suffix. | `packet.go` |
| `tnc2ax25` | One-way checked adapter from `tnc2.TNC2Packet` to strict classic `ax25.Frame`; the only layer allowed to impose AX.25 callsign/SSID limits on the textual model. | `adapter.go` |
| `rxttelemetry` | Versioned LoRa APRS JSON NDJSON consumer plus legacy `/rxt.json` polling. `RawReception` preserves every accepted `rx`; parsed TNC2 and RXT/local link projections remain optional and independent. | `service.go` |
...
| `aprs` | APRS info-field parsing (positions, messages, weather, telemetry, mic-e, plus assorted extensions -- see [glossary.md](glossary.md)). `ParseTNC2Packet` decodes semantics from the textual transport without requiring AX.25 conversion. | `parse.go`, `position.go`, `mice.go`, `message.go`, `weather.go`, ... |
...
|---|---|---|
| `weather` / `weather/davis` | Hardware-independent `Observation`, common APRS complete-weather encoder, WxNow.txt adapter, native Davis Vantage driver, and native Peet Bros Ultimeter serial reader. `davis/protocol.go` ports the 99-byte LOOP/LOOP2 decoder, CRC-16, sentinels and conversions; `davis/serial.go` owns the wake → `LPS 3 2` → ACK → alternating-packet exchange. `weather/peet.go` reads and validates continuous `!!` Data Logger or `$ULTW` Packet Mode records at 2400 8N1. | [`../handbook/beacons.html`](../handbook/beacons.html) |
| `beacon` | Position/object/tracker/weather/custom/igate beacon scheduler (min-heap), smart-beacon, encoder. Weather beacons select `davis_serial`, `peet_serial`, or `wxnow_file`, receive a `weather.Observation`, run the common APRS encoder, and build a positioned `_` report. The Beacons UI exposes source-specific settings plus custom `custom_info` and `comment_cmd` for non-weather beacons; form rules live in `web/src/lib/beaconForm.js`. Weather and custom info follow the normal RF/APRS-IS routes and use direct textual output on the authorized Native TNC2 channel. Per-beacon `position_format` column ('compressed'\|'uncompressed'\|'mic_e', added in configstore migration 23) selects the wire encoding; uncompressed and Mic-E beacons honor `ambiguity` 0..4 via `aprs.ApplyLatLonAmbiguity` and the K/L/Z destination variants. Encoder entry points: `pkg/beacon/builder.go` switches on `Format`; `PositionInfo` (uncompressed, APRS101 ch 6) and `CompressedPositionInfo` (APRS101 ch 9) live in `pkg/beacon/encoder.go`; `MicEPositionInfo` + `MicEDestination` (APRS101 ch 10) live in `pkg/beacon/mice.go`. Mic-E swaps the AX.25 destination with the lat-derived destination at frame-build time in `scheduler.go`. | [`../handbook/beacons.html`](../handbook/beacons.html) |
...
| `remoteactions` | OUTBOUND counterpart: macro + remote-OTP credential stores, RFC 6238 TOTP generation, plus CA2RXU `!RC1` HMAC command credentials, durable sender counters and envelope generation. | [`remote-actions.md`](remote-actions.md) |
...
| Package | Purpose | Handbook |
|---|---|---|
| `configstore` | SQLite config DB (GORM, glebarez/sqlite, pure Go); migrations, seeds, models. Actions tables live here too: `actions`, `otp_credentials`, `action_listener_addressees`, `action_invocations` (migration 15, raw SQL — not AutoMigrate; see [`actions.md`](actions.md)). Outbound Actions adds `remote_otp_credentials`, `remote_action_macros` (migration 16) and `remote_command_credentials` for CA2RXU HMAC keys/counters (migration 30; see [`remote-actions.md`](remote-actions.md)). | [`../handbook/preferences.html`](../handbook/preferences.html) |
...
| `src/components/messages/remote_actions/` | Outbound Actions UI: `RemoteActionsDrawer` (zap-icon-anchored thread drawer), TOTP macro/free-form controls, and `RemoteCommandPanel` + `RemoteCommandCredentialModal` for CA2RXU `!RC1` HMAC control. See [`remote-actions.md`](remote-actions.md). |
