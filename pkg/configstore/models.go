...
// TNC2Config is the operator-managed direct textual transceiver. A missing
// row means the legacy command-line flags remain in force.
type TNC2Config struct {
	ID           uint32    `gorm:"primaryKey" json:"-"`
	TCPAddress   string    `gorm:"not null;default:''" json:"tcp_address"`
	SerialDevice string    `gorm:"not null;default:''" json:"serial_device"`
	SerialBaud   uint32    `gorm:"not null;default:115200" json:"serial_baud"`
	TXTransport  string    `gorm:"not null;default:''" json:"tx_transport"`
	TXSource     string    `gorm:"not null;default:''" json:"tx_source"`
	TXChannel    uint32    `gorm:"not null;default:1" json:"tx_channel"`
	MaxTXBytes   uint32    `gorm:"not null;default:255" json:"max_tx_bytes"`
	CreatedAt    time.Time `json:"-"`
	UpdatedAt    time.Time `json:"-"`
}
